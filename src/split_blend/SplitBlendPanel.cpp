...
// event ID's for SplitBlendPanel
enum
{
    ID_Quit = 1,
    ID_About   
...
BEGIN_EVENT_TABLE(SplitBlendPanel, wxPanel)
    EVT_END_PROCESS(-1, SplitBlendPanel::OnProcessTerminate)
END_EVENT_TABLE()

SplitBlendPanel::SplitBlendPanel(wxWindow * parent)
    : wxPanel(parent)
{
	m_paused=false;
	m_overwrite=false;
...
bool SplitBlendPanel::StitchProject(wxString scriptFile, wxString outname,
...
bool SplitBlendPanel::DetectProject(wxString scriptFile, 
...
void SplitBlendPanel::CancelStitch()
{
    DEBUG_TRACE("");
    m_execPanel->KillProcess();
}

bool SplitBlendPanel::IsPaused()
{
	return m_paused;
}

void SplitBlendPanel::SetOverwrite(bool over)
{
	m_overwrite = over;
}

void SplitBlendPanel::PauseStitch()
{
	m_paused=true;
	m_execPanel->PauseProcess();
}

void SplitBlendPanel::ContinueStitch()
{
	m_execPanel->ContinueProcess();
	m_paused=false;
}

long SplitBlendPanel::GetPid()
{
	return m_execPanel->GetPid();
}

bool SplitBlendPanel::SaveLog(const wxString &filename)
{
