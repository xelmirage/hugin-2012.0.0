                                   ...
                                   HuginBase::PanoramaMakefilelibExport::PTPrograms progs, bool doDeleteOnExit)
{
	...
	::wxMessageBox(*it+"\n"+*it_out,"output");
	if (! m_stitchPanel->StitchProject(*it, *it_out, progs)) {
		return false;
	}
	m_isStitching = true;
...
bool split_blend_App::OnInit()
{
	...
	SplitBlendFrame *frame = new SplitBlendFrame(NULL, wxT("SplitBlender"), wxDefaultPosition, wxSize(640,600) );
    frame->Show( true );
    SetTopWindow( frame );
