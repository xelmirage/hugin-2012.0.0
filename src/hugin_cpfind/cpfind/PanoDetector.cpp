			...
			case GPS:
				{
					cout <<"*********************GPS***********************"<<endl;
					std::vector<HuginBase::UIntSet> imgPairs(_panoramaInfo->getNrOfImages());
                    if(!matchgps(aExecutor, imgPairs))
                    {
                        return;
                    };
...
bool PanoDetector::matchgps(PoolExecutor& aExecutor, std::vector<HuginBase::UIntSet> &checkedPairs)
{
    // 3. prepare matches
    _matchesData.clear();
	...
	//_inputFile="h:\\bin\\kl\\all\\01.txt";
	//POINTSL pointsL;
	...
	//if (data.bad())
	//{
	//	cerr << "ERROR: could not open gps file: '" << _inputFile << "'!" << endl;
	//	return false;
	//}
	//TRACE_INFO(endl<< "--- paring with gps ---" << endl);
	...
	//	pointsL.push_back(PointL(UTMEasting,UTMNorthing,yaw));;
	...
	int id;
	unsigned int i1,i2;
    for (i1 = 0; i1 < _filesData.size(); ++i1)
    {
		...
		cout<<_filesData[i1]._name<<endl;
		Exiv2::Image::AutoPtr image = Exiv2::ImageFactory::open(_filesData[i1]._name);
		assert (image.get() != 0);
		image->readMetadata();
		Exiv2::ExifData &exifData = image->exifData();
		string tag=boost::lexical_cast<string>(exifData["Exif.Photo.UserComment"]);
		split(SplitVec,tag,is_any_of(" "),token_compress_on);
		id=boost::lexical_cast<int>(SplitVec[1]);
		ids.push_back(id);
		vector<int> ipair;
		ipair.push_back(id);
		...
		///**************************added for GPS*********/
		//cout<<i1<<"      "<<pointsL[i1].x<<"  "<<pointsL[i1].y<<endl;
		//int id=i1+1;
		//DISTS dists;
		//dists.clear();
		//
		//for(unsigned int j=i1+1;j<aEnd;++j)
		//{
		//	long d=pointsL[i1].distance(pointsL[j]);
		//	dist tempDist(id,d,0);
		//	if(tempDist.distance >0&&tempDist.distance <1000)
		//	{
		//		if (abs(pointsL[i1].yaw-pointsL[j].yaw)<10)
		//		{
		//			if ((j-i1)<=1)
		//			{
		...
		//			}
		//		}
		//		else if (abs((abs(pointsL[i1].yaw-pointsL[j].yaw)-180))<10)
		//		{
		//			dists.push_back(tempDist);
  ...
  //          checkedPairs[i2].insert(i1);
		...
		//for (unsigned int i2 = (i1+2*(29-i1%29)-2); i2 < (i1+2*(29-i1%29)+2); ++i2)
...
bool PanoDetector::matchMultiRow(PoolExecutor& aExecutor)
{
    ...
    if(n>1)
    {
        ...
        {
			//for(unsigned int j=i+30; j<i+50; j++)
            for(unsigned int j=i+1; j<ImagesGroups.size(); j++)
            {
				//if (j>=ImagesGroups.size()) break;
