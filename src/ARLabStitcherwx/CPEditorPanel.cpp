...
//    DEBUG_ASSERT(m_rightImg->GetState == NEW_POINT_SELECTED);
    ...
    SelectGlobalPoint(lPoint);
    changeState(NO_POINT);
	ARLabStitcherwxMainFrame::Get()->SetStatusText(_("new control point added"));
...
//    DEBUG_DEBUG("automatically estimating point in other window");
        ...
        && p.y < (int) pImg.getSize().height() && p.y >= 0)
    {
        ...
        if (m_fineTuneCB->IsChecked()) {
            ARLabStitcherwxMainFrame::Get()->SetStatusText(_("searching similar points..."),0);
            ...
            if (! corrOk) {
                ...
                if (!m_autoAddCB->IsChecked()) {
                    ...
                    ARLabStitcherwxMainFrame::Get()->SetStatusText(s2,0);
                } else {
                    // add point
                    otherImg->setNewPoint(corrPoint.maxpos);
                    changeState(BOTH_POINTS_SELECTED);
        ...
        // do nothing special
        wxBell();
        ARLabStitcherwxMainFrame::Get()->SetStatusText(_("Estimated point outside image"),0);
    }
}
        ...
        if (cpCreationState == OTHER_POINT) {
            ...
            if (m_fineTuneCB->IsChecked() ) {
                ...
                if (! corrOk) {
                    ...
                    ARLabStitcherwxMainFrame::Get()->SetStatusText(corrMsg,0);
...
//    wxImage wxTmplImg;
    ...
    ARLabStitcherwxMainFrame::Get()->SetStatusText(wxString::Format(_("Point fine-tuned, angle: %.0f deg, correlation coefficient: %0.3f, curvature: %0.3f %0.3f "),
...
void CPEditorPanel::changeState(CPCreationState newState)
{
    ...
    case LEFT_POINT:
        ...
        m_addButton->Enable(false);
        m_delButton->Enable(false);
        ARLabStitcherwxMainFrame::Get()->SetStatusText(_("Select point in right image"),0);
        break;
    case RIGHT_POINT:
        ...
        m_addButton->Enable(false);
        m_delButton->Enable(false);
        ARLabStitcherwxMainFrame::Get()->SetStatusText(_("Select point in left image"),0);
        break;
    ...
    case BOTH_POINTS_SELECTED:
        ...
        m_delButton->Enable(false);
//        ARLabStitcherwxMainFrame::Get()->SetStatusText(_("change points, or press right mouse button to add the pair"));
    }
    // apply the change
...
//        ARLabStitcherwxMainFrame::Get()->SetStatusText(_("searching for cloud-like control points..."),0);
//        progress.increaseProgress(1.0, std::wstring(wxString(_("Loading model file")).wc_str(wxConvLocal)));
//
////        struct celeste::svm_model* model=ARLabStitcherwxMainFrame::Get()->GetSVMModel();
//        if(model==NULL)
//        {
//            ARLabStitcherwxMainFrame::Get()->SetStatusText(wxT(""),0);
//            return;
//        };
//
