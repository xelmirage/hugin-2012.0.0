		...
		*/
			...
			AppBase::MultiProgressDisplay & progress)
		{
			...
			public:
				...
				float datum_to_float(Exiv2::Exifdatum data)
				{
					float result=0;
					result=data.toFloat(0)+data.toFloat(1)/60+data.toFloat(2)/3600;
					return result;
				}

				virtual void stitch(const PanoramaOptions & opts, UIntSet & images,
					const std::string & basename,
					SingleImageRemapper<ImageType, AlphaType> & remapper,bool findcenter=true)
				{
