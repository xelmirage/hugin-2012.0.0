...
{
    // parse arguments
    const char* optstring = "g:o:p:f:c:s:lh";
            ...
            do
            {
                ...
                if((finddata.attrib & _A_SUBDIR)==0)
                {
                    ...
                    if(usegps)
					{
						try
						{
							Exiv2::Image::AutoPtr image = Exiv2::ImageFactory::open(std::string(newFile));
							assert (image.get() != 0);
							image->readMetadata();
							...
							if(SplitVec[0].compare("selected")==0)
							{
								cout<<std::string(newFile)<<":"<<tag<<endl;
